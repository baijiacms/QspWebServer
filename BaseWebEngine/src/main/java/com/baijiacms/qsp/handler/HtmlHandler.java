...
public class HtmlHandler {
    ...
    public static final String HTML_CONTENT_TYPE = "text/html;charset=" + QspConstants.CHARSET_STR;
    public static final String JSON_CONTENT_TYPE = "application/json;charset=" + QspConstants.CHARSET_STR;
    private EngineHandler engineHandler;
    public HtmlHandler(EngineHandler engineHandler) {
        this.engineHandler = engineHandler;
    }
    public HtmlHandler(int userId,EngineHandler engineHandler) {
        this.mPlayerEngine = new PlayerEngine(userId);
        this.engineHandler = engineHandler;
    }
    public void handleRequest(URL url, String target, HttpServletResponse response) throws IOException {
...
//                return;
            default:
                if (target.startsWith("/engine/lib/")) {
                    byteResultStream = StreamUtils.getEngineResourceInputSteam(null, target);
                    StreamUtils.copy(byteResultStream, response.getOutputStream());
                    ResponseUtil.setContentType(response, MyMediaTypeFactory.getContentType(target));
                    return;
                }
        }
        byteResultStream = StreamUtils.getGameResourceInputSteam(null, target);
        StreamUtils.copy(byteResultStream, response.getOutputStream());
        ResponseUtil.setContentType(response, MyMediaTypeFactory.getContentType(target));
        return;
    }
    public void handleSession(URL url, String target, HttpServletResponse response) throws IOException {
...
//        target = target.replace(QspConstants.HTTP_LOCAL_URL, "");
        ...
        byteResultStream = StreamUtils.getGameResourceInputSteam(mPlayerEngine, target);
        StreamUtils.copy(byteResultStream, response.getOutputStream());
        ResponseUtil.setContentType(response, MyMediaTypeFactory.getContentType(target));
        return;
    }
